#pragma once

#include <Arduino.h>
#include <cstdint>
#include <vector>
#include "mode/mode.h"
#include "signs.h"
#include "constants.h"

class NewScreen {
    public:
        static NewScreen& getInstance();
        // Prohibiting copying
        NewScreen& operator=(const NewScreen&) = delete;
        NewScreen(const NewScreen&) = delete;
        


        void clear();
        void setup();
        void render();
        std::vector<int> readBytes(std::vector<int> bytes);
        // Per-pixel manipulation
        void setPixel(uint8_t x, uint8_t y, uint8_t value, uint8_t brightness = 255);
        void setPixelAtIndex(uint8_t index, uint8_t value, uint8_t brightness = 255);
        // Drawing methods
        void drawLine(int x1, int y1, int x2, int y2, int ledStatus, uint8_t brightness);
        void drawRectangle(int x, int y, int width, int height, bool fill, int ledStatus, uint8_t brightness);
        void drawCharacter(int x, int y, std::vector<int> bits, int bitCount, uint8_t brightness);
        void drawNumbers(int x, int y, std::vector<int> numbers, uint8_t brightness);
        void drawBigNumbers(int x, int y, std::vector<int> numbers, uint8_t brightness);
        void drawWeather(int x, int y, int weather, uint8_t brightness);
        // Buffer methods
        void setBuffer(const uint8_t* buffer);
        const uint8_t* getBuffer() const;
        uint8_t getBufferIndex(int index);

    private:
        // Make singleton class
        NewScreen() = default;

        // NOTE: Skip global brightness for now, but keep a [0,255] per-LED brightness
        uint8_t buffer_[ROWS * COLS];
        uint8_t pixelMap_[ROWS * COLS] = {
            
        };
        static void renderOnTimer();
        static const uint8_t grayLevels = 64;
        uint8_t brightnessMap_[grayLevels];

};

extern NewScreen& NScreen;
